...
import alshariqa.althaqafiya.magazine.ui.article.adapters.SliderPagerAdapter
...
class ArticleDetailFragment : BaseFragment(), ArticlesViewModel.View {
        ...
        mArticlesViewModel.let {
            ...
            arguments?.run {
                article = getParcelable<Article>(Keys.ARTICLE_DETAIL)?.apply {
                    ...
                    ImageLoader.load(authorPhoto, ivAuthor1, R.drawable.ic_ph_author)
                    tvTitle.text = title
                    tvSubTitle.text = subTitle
                    tvDate.text = publishDate
                    tvAuthor.text = authorName
                    tvAuthor1.text = authorName
                    tvArticle.text = title
                    val sliderPagerAdapter = SliderPagerAdapter(this@ArticleDetailFragment)
                    vpArticle.adapter = sliderPagerAdapter
    ...
    override fun onArticles(articles: List<Article>) {
        ...
        rv.adapter = ArticlesAdapter(articles, mArticlesViewModel, true)
    }
